...
from dataset.eval_PST900 import inference
import numpy as np
...
parser.add_argument('--data-dir', default='/workspace/try1/PST900/train', metavar='DIR',
                    help='path to MFNet')
...
parser.add_argument('--momentum', default=0.9, type=float, metavar='M',
                    help='momentum')
parser.add_argument('--num_class', default=5, type=int, metavar='N',
...
def train():
    ...
    train_data = data.MSRS(transform=transforms.Compose([data.RandomFlip(), data.ToTensor(), data.RandomHSV(), data.RandomResizedCrop((720, 1280), (0.5, 2.0)), data.Normalize()]), phase_train=True, data_dir=args.data_dir)
    ...
    metrics = AverageMeter(5, 255, device)
    ...
    for epoch in range(int(args.start_epoch), args.epochs):
        ...
        for i, sample in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            pred_scales = model(image, thermal)
            ...
            # else:
            loss = weighted(pred_scales, target)
        ...
        if local_count % 5 == 0:
            bg_iou, fire_iou, backpack_iou, hand_iou, survivor_iou = ious
            bg_acc, fire_acc, backpack_acc, hand_acc, survivor_acc = Accuracy
            bg_iou_tr, fire_iou_tr, backpack_iou_tr, hand_iou_tr, survivor_iou_tr = ious_train
            bg_acc_tr, fire_acc_tr, backpack_acc_tr, hand_acc_tr, survivor_acc_tr = acc_train
            logger.info('***** Train *****')
            logger.info('IoU for each category >>> background:{:.2f}, fire-extinguisher:{:.2f}, backpack:{:.2f}, hand-drill:{:.2f}, survivor:{:.2f}'.format(bg_iou_tr, fire_iou_tr, backpack_iou_tr, hand_iou_tr, survivor_iou_tr))
            logger.info('accuracy for each category >>> background:{:.2f}, fire-extinguisher:{:.2f}, backpack:{:.2f}, hand-drill:{:.2f}, survivor:{:.2f}'.format(bg_acc_tr, fire_acc_tr, backpack_acc_tr, hand_acc_tr, survivor_acc_tr))
            logger.info("***** Evaluation *****")
            logger.info('IoU for each category >>> background:{:.2f}, fire-extinguisher:{:.2f}, backpack:{:.2f}, hand-drill:{:.2f}, survivor:{:.2f}'.format(bg_iou, fire_iou, backpack_iou, hand_iou, survivor_iou))
            logger.info('accuracy for each category >>> background:{:.2f}, fire-extinguisher:{:.2f}, backpack:{:.2f}, hand-drill:{:.2f}, survivor:{:.2f}'.format(bg_acc, fire_acc, backpack_acc, hand_acc, survivor_acc))
        logger.info('***** Train ***** >>>> meanIOU: {:.2f}, mAcc: {}\n'.format(miou_train, macc_train))
        logger.info('***** Evaluation ***** >>>> loss: {:.2f}, meanIOU: {:.2f}, mAcc: {}\n'.format(loss_val, MIOU, mAcc))
